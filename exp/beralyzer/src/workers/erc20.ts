...
export async function ingestErc20Registry(pg: Client, cfg: Erc20WorkerConfig): Promise<void> {
  const provider = new ethers.JsonRpcProvider(cfg.elRpcUrl);

  // Cursor
  const curRes = await pg.query('SELECT last_processed_height FROM ingest_cursors WHERE module=$1', ['erc20_registry']);
  const last = curRes.rows[0] ? Number(curRes.rows[0].last_processed_height) : 0;
  ...
  for (const row of q.rows) {
    ...
    const exists = await pg.query('SELECT 1 FROM erc20_tokens WHERE address=$1', [addr]);
