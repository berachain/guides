...
export async function ingestEl(pg: Client, cfg: ElWorkerConfig & { log?: boolean }): Promise<void> {
  const provider = new ethers.JsonRpcProvider(cfg.elRpcUrl);

  // Load cursor
  const curRes = await pg.query('SELECT last_processed_height FROM ingest_cursors WHERE module=$1', ['blocks_el']);
  ...
  for (let from = start; from <= end; from += cfg.blockBatchSize) {
    ...
    for (const { bn, blk } of blocks) {
      ...
      const gasLimit = (blk as any).gasLimit ? BigInt((blk as any).gasLimit) : null;
      await pg.query(
    ...
    if (cfg.log) console.log(`EL: blocks ${from}-${to} inserted in ${Date.now() - t0}ms`);
    ...
    for (const { bn, blk } of blocks) {
      ...
      for (let i = 0; i < txHashes.length; i += cfg.txConcurrency) {
        ...
        for (const item of items) {
          ...
          const selector = tx.data && tx.data.length >= 10 ? ('0x' + tx.data.slice(2, 10)).toLowerCase() : null;
      ...
      if (txWithPrices > 0) {
        ...
        await pg.query(
          ...
          [bn, totalFees.toString(), totalPriorityFees.toString(), avgEff.toString(), avgPrio.toString()]
        );
      }
    }
  ...
  if (cfg.log) console.log(`EL: window ${start}-${end} completed in ${Date.now() - t0All}ms`);
}
