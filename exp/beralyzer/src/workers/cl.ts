...
async function getValidators(clUrl: string, height: number): Promise<ValidatorsAtHeight | null> {
  try {
...
export async function ingestClAbsences(pg: Client, cfg: ClWorkerConfig & { log?: boolean }): Promise<void> {
  const latest = await getLatestHeight(cfg.clRpcUrl);
  const curRes = await pg.query('SELECT last_processed_height FROM ingest_cursors WHERE module=$1', ['cl_absences']);
  ...
  for (let from = start; from <= end; from += cfg.batchSize) {
    ...
    for (let h = from; h <= to; h++) {
      ...
      const round = typeof roundRaw === 'string' ? parseInt(roundRaw, 10) : (typeof roundRaw === 'number' ? roundRaw : null);
      ...
      const missingPct = totalVotingPower > 0n ? Number((missingVotingPower * 10000n) / totalVotingPower) / 100 : 0;
      ...
      await pg.query(
        `UPDATE blocks SET missing_count=$2, missing_voting_power=$3, total_voting_power=$4, missing_percentage=$5, last_commit_round=$6, absent_validators=$7
         WHERE height=$1`,
        [h, missingCount, missingVotingPower.toString(), totalVotingPower.toString(), missingPct, round, JSON.stringify(absentees)]
      );
    ...
    if (cfg.log) console.log(`CL: absences ${from}-${to} in ${Date.now() - t0}ms`);
  }
  if (cfg.log) console.log(`CL: window ${start}-${end} in ${Date.now() - t0All}ms`);
}
