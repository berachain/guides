...
export function classifyClient(clientString: string): { type: string; version: string; full: string } {
...
export function decodeExtraDataAscii(extraDataHex: string | null | undefined): string {
  if (!extraDataHex || extraDataHex === '0x') return '';
  const hex = extraDataHex.startsWith('0x') ? extraDataHex.slice(2) : extraDataHex;
  try {
    const bytes: number[] = [];
