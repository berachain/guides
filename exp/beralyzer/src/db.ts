...
export async function getCursor(pg: Client, module: string): Promise<number | null> {
  const { rows } = await pg.query('SELECT last_processed_height FROM ingest_cursors WHERE module=$1', [module]);
...
export async function upsertCursor(pg: Client, module: string, height: number): Promise<void> {
  await pg.query(
