...
export function UnsupportedBanner({ className }: UnsupportedBannerProps) {
  return (
    <div
      ...
      role="status"
    >
      <p className="font-medium text-amber-200">Execution permissions API not available</p>
      <p className="mt-2 leading-relaxed text-amber-100/90">
        This wallet does not expose{' '}
        <code className="rounded bg-amber-500/20 px-1 py-0.5 font-mono text-xs text-amber-200">wallet_getSupportedExecutionPermissions</code> (JSON-RPC
        -32601 method not found). Install{' '}
        <a
