  ...
  return (
    <section className="rounded-xl border border-l-4 border-[#2A2A2A] border-l-fuchsia-500 bg-[#1A1A1A] p-6">
      ...
      <p className="mt-4 text-sm leading-relaxed text-zinc-400">
        Build a <code className="font-mono text-xs">PermissionRequest</code> and
        call{" "}
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-5 lg:gap-10">
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <div className="text-sm leading-relaxed text-zinc-400">
            <p>
              When the wallet approves, it returns a <strong>context</strong>{" "}
            ...
            <p className="mt-3">
              <strong>delegationManager</strong> is the contract you call (per
              ERC-7710) to redeem—your session account submits the encoded
              execution together with that context.
            </p>
            <p className="mt-3">
              <strong>dependencies</strong> describes accounts that still need
              deployment: each item includes a{" "}
              <code className="font-mono text-xs">factory</code> and{" "}
              <code className="font-mono text-xs">factoryData</code> you can use
              to deploy them before redemption succeeds.
            </p>
            <p className="mt-3">
              Defaults pick a supported permission from Phase 1, pre-fill a
              one-hour <code className="font-mono text-xs">expiry</code> rule,
