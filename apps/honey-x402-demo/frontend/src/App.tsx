...
function App() {
  ...
  const honeyAddress = (
    deployments.honey && deployments.honey !== ZERO_ADDR
      ? deployments.honey
      : ZERO_ADDR
  ) as `0x${string}`;
  const demoAddress = (
    deployments.demo && deployments.demo !== ZERO_ADDR
      ? deployments.demo
      : ZERO_ADDR
  ) as `0x${string}`;
  const permit2Address = (
    deployments.permit2 && deployments.permit2 !== ZERO_ADDR
      ? deployments.permit2
      : ZERO_ADDR
  ) as `0x${string}`;
    ...
    <div className="app">
      <header>
        ...
        <p className="subtitle-detail">
          Private keys loaded from <code>.env</code> — no wallet extension
          needed
        </p>
      </header>
      ...
      {(!deployments.honey ||
        honeyAddress === ZERO_ADDR ||
        permit2Address === ZERO_ADDR) && (
      ...
      <footer>
        ...
        <ol>
          <li>
            <strong>Wallet A</strong> ({tokenHolderAccount.address.slice(0, 8)}
            ...) holds HONEY tokens. Click the first button to sign an off-chain
            message — <em>no gas is spent</em>.
          </li>
          <li>
            <strong>Wallet B</strong> (
            {gasSubsidizerAccount.address.slice(0, 8)}...) acts as the relayer.
            Click the second button to submit the signed message on-chain —{" "}
            <em>Wallet B pays for gas</em>.
          </li>
          <li>
            Each demo section shows both steps separately so you can see the
            gasless flow in action.
          </li>
        </ol>
        <p className="footer-note">
          Both private keys are loaded from the project root <code>.env</code>{" "}
          file (<code>PRIVATE_KEY</code> and{" "}
          <code>PRIVATE_KEY_GAS_SUBSIDIZER</code>). No browser wallet extension
          is needed.
        </p>
      </footer>
    </div>
