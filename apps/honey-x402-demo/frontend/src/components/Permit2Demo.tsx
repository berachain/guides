...
interface SigTransferData {
  sig: `0x${string}`; token: `0x${string}`; amount: bigint
  nonce: bigint; deadline: bigint; spender: `0x${string}`
}

interface AllowTransferData {
  sig: `0x${string}`; token: `0x${string}`; amount: number
  expiration: number; nonce: number; spender: `0x${string}`; sigDeadline: bigint
}
...
export function Permit2Demo({ honeyAddress, permit2Address, demoAddress, onTxComplete }: Permit2DemoProps) {
  ...
  const permit2Enabled = enabled && permit2Address !== '0x0000000000000000000000000000000000000000'
  ...
  const { data: honeyBalA } = useReadContract({ address: honeyAddress, abi: erc20Abi, functionName: 'balanceOf', args: [tokenHolderAccount.address], query: { enabled, refetchInterval: 4000 } })
  const { data: ethBalB } = useBalance({ address: gasSubsidizerAccount.address, query: { refetchInterval: 4000 } })
  const { data: honeyBalB } = useReadContract({ address: honeyAddress, abi: erc20Abi, functionName: 'balanceOf', args: [gasSubsidizerAccount.address], query: { enabled, refetchInterval: 4000 } })
  ...
  const handleSignSignature = async () => {
    setError(null); setStep('signing'); setStatus('Wallet A signing Permit2 off-chain...')
    try {
      ...
      const sig = await tokenHolderClient.signTypedData({
        ...
        message: { permitted: { token: honeyAddress, amount: value }, spender: demoAddress, nonce, deadline },
      })

      setSigData({ sig, token: honeyAddress, amount: value, nonce, deadline, spender: demoAddress })
      setSignedSig(sig)
  ...
  const handleSignAllowance = async () => {
    setError(null); setStep('signing'); setStatus('Wallet A signing Permit2 allowance off-chain...')
    try {
      ...
      const sig = await tokenHolderClient.signTypedData({
        ...
        message: { details: { token: honeyAddress, amount: amountValue, expiration, nonce }, spender: demoAddress, sigDeadline },
      })

      setAllowData({ sig, token: honeyAddress, amount: amountValue, expiration, nonce, spender: demoAddress, sigDeadline })
      setSignedSig(sig)
      setStep('signed'); setStatus('')
    } catch (err) { setError(`Signing failed: ${(err as Error).message}`); setStep('idle'); setStatus('') }
  }
  ...
  const handleExecSig = async () => {
    ...
    try {
      const hash = await gasSubsidizerClient.writeContract({
        address: demoAddress, abi: demoAbi, functionName: 'transferFromWithPermit2Signature',
        args: [
          { permitted: { token: sigData.token, amount: sigData.amount }, nonce: sigData.nonce, deadline: sigData.deadline },
          { to: gasSubsidizerAccount.address, requestedAmount: sigData.amount },
          tokenHolderAccount.address, sigData.sig,
        ],
      })
      setStep('confirming'); setStatus('Waiting for confirmation...')
      const r = await publicClient.waitForTransactionReceipt({ hash })
      setReceipt(r); setStep('done'); setStatus(''); onTxComplete?.()
  ...
  const handleExecAllow = async () => {
    ...
    try {
      const hash = await gasSubsidizerClient.writeContract({
        address: demoAddress, abi: demoAbi, functionName: 'transferFromWithPermit2Allowance',
        args: [
          tokenHolderAccount.address,
          { details: { token: allowData.token as `0x${string}`, amount: allowData.amount, expiration: allowData.expiration, nonce: allowData.nonce }, spender: allowData.spender as `0x${string}`, sigDeadline: allowData.sigDeadline },
          allowData.sig,
  ...
  const handleSign = method === 'signature' ? handleSignSignature : handleSignAllowance
  const handleExecute = method === 'signature' ? handleExecSig : handleExecAllow

  const handleReset = () => {
    setStep('idle'); setSignedSig(null); setSigData(null); setAllowData(null); setReceipt(null); setError(null); setStatus('')
  }
    ...
    <div className="demo-section">
      ...
      <div className="demo-content">
        <div className="explanation">
          <p><strong>How it works:</strong> Uniswap's Permit2 is a <strong>smart contract deployed on-chain</strong> that acts as a universal approval layer for any ERC20 token. The token holder signs a Permit2 message off-chain, and the gas subsidizer submits it to the Permit2 contract on-chain.</p>
          <p className="highlight">Key: Works with any ERC20 — no need for the token to implement EIP-2612.</p>
          <p className="note"><strong>Permit2 Contract:</strong> <code className="mono">{permit2Address}</code></p>
        </div>
        ...
        {!hasPermit2Approval && permit2Enabled && (
          <div className="detail-card setup-required">
            ...
            <p className="setup-desc">
              Before Permit2 can transfer tokens on behalf of Wallet A, Wallet A must do a
              one-time ERC20 <code>approve()</code> to the Permit2 contract. This is a standard
              <code>approve</code> transaction that costs gas — but only needs to happen once.
              After this, all Permit2 operations are gasless for Wallet A.
            </p>
            <div className="detail-grid">
              ...
              <span className="detail-val mono">Permit2 ({addr(permit2Address)})</span>
            ...
            <button
              ...
              className="setup-btn"
            >
              {approving ? '⏳ Approving...' : 'Wallet A: Approve Permit2 (One-time Setup)'}
            </button>
          </div>
        )}
        ...
        {step === 'idle' && (
          <>
            ...
            <div className="input-group">
              <label>Method:</label>
              <select value={method} onChange={(e) => { setMethod(e.target.value as 'signature' | 'allowance'); handleReset() }}>
                <option value="signature">Signature Transfer (One-time, gasless)</option>
                <option value="allowance">Allowance Transfer (Reusable, time-bound)</option>
              </select>
            </div>
            <div className="input-group">
            ...
            {method === 'signature' && (
              <div className="detail-card preview">
                ...
                <div className="detail-grid">
                  ...
                  <span className="detail-val">Permit2 <code>PermitTransferFrom</code></span>
                  ...
                  <span className="detail-key">Amount</span>
                  <span className="detail-val">{amount} HONEY (exact, one-time)</span>
              ...
              <>
                <div className="detail-card time-bound">
                  ...
                  <p className="time-bound-desc">
                    The Allowance Transfer sets a <strong>spending cap</strong> that the spender
                    can draw from <strong>multiple times</strong> — until either the cap is exhausted
                    or the allowance expires. Two separate timers control the window:
                  </p>
                  ...
                  <div className="time-bound-controls">
                    <div className="time-bound-row">
                      ...
                      <select
                        ...
                        onChange={(e) => setAllowanceDuration(Number(e.target.value))}
                    ...
                    <div className="time-bound-row">
                      ...
                      <select
                        ...
                        onChange={(e) => setSigDeadlineDuration(Number(e.target.value))}
                        className="time-bound-select"
                      >
                        {DURATION_OPTIONS.map(o => (
                          <option key={o.value} value={o.value}>{o.label}</option>
                ...
                <div className="detail-card preview">
                  ...
                  <div className="detail-grid">
                    <span className="detail-key">Type</span>
                    <span className="detail-val">Permit2 <code>PermitSingle</code></span>
                    <span className="detail-key">From (owner)</span>
                    <span className="detail-val mono">{tokenHolderAccount.address}</span>
                    <span className="detail-key">Spender</span>
                    <span className="detail-val mono">{demoAddress}</span>
                    <span className="detail-key">Spending cap</span>
                    <span className="detail-val">{amount} HONEY (uint160 max)</span>
                    <span className="detail-key">Allowance expires</span>
                    <span className="detail-val">{fmtTimestamp(previewExpiration)} ({fmtDuration(allowanceDuration)})</span>
                    <span className="detail-key">Sig deadline</span>
                    <span className="detail-val">{fmtTimestamp(previewSigDeadline)} ({fmtDuration(sigDeadlineDuration)})</span>
                    <span className="detail-key">Nonce</span>
                    <span className="detail-val">0 (sequential — Permit2 tracks per-token)</span>
                    <span className="detail-key">Reusable?</span>
                    <span className="detail-val">Yes — until cap exhausted or expired</span>
                    <span className="detail-key">Gas cost</span>
                    <span className="detail-val free">FREE (off-chain signature)</span>
                  </div>
                </div>
                ...
                <div className="detail-card comparison-hint">
                  ...
                  <table className="mini-compare">
                    <thead>
                      <tr><th></th><th>Signature Transfer</th><th>Allowance Transfer</th></tr>
                    </thead>
                    <tbody>
                      <tr><td>Uses</td><td>One-time only</td><td className="highlight-cell">Reusable until cap/expiry</td></tr>
                      <tr><td>Amount type</td><td>uint256 (exact)</td><td className="highlight-cell">uint160 (spending cap)</td></tr>
                      <tr><td>Expiration</td><td>Deadline only</td><td className="highlight-cell">Allowance expiry + sig deadline</td></tr>
                      <tr><td>Nonce</td><td>Random</td><td className="highlight-cell">Sequential (per token)</td></tr>
                      <tr><td>Best for</td><td>Single transfers</td><td className="highlight-cell">Recurring / subscription-like</td></tr>
                    </tbody>
                  </table>
                </div>
              </>
          ...
          <>
            {method === 'signature' && sigData && (
              <div className="detail-card sig">
                ...
                <div className="detail-grid">
                  ...
                  <span className="detail-val">One-time Signature Transfer</span>
                  <span className="detail-key">Amount</span>
                  <span className="detail-val">{amount} HONEY</span>
                  <span className="detail-key">Nonce</span>
                  <span className="detail-val">{sigData.nonce.toString()} (random)</span>
                  <span className="detail-key">Deadline</span>
                  <span className="detail-val">{fmtTimestamp(Number(sigData.deadline))}</span>
                  <span className="detail-key">Raw signature</span>
                  <span className="detail-val mono break tiny">{signedSig}</span>
            ...
            {method === 'allowance' && allowData && (
              <div className="detail-card sig">
                ...
                <div className="detail-grid">
                  <span className="detail-key">Method</span>
                  <span className="detail-val">Reusable Allowance Transfer</span>
                  <span className="detail-key">Spending cap</span>
                  <span className="detail-val">{amount} HONEY</span>
                  <span className="detail-key">Nonce</span>
                  <span className="detail-val">{allowData.nonce} (sequential)</span>
                </div>
                ...
                <div className="time-bound-signed">
                  ...
                  <div className="detail-grid">
                    <span className="detail-key">Allowance expires</span>
                    <span className="detail-val">{fmtTimestamp(allowData.expiration)} ({fmtDuration(allowData.expiration - Math.floor(Date.now() / 1000))} remaining)</span>
                    <span className="detail-key">Sig deadline</span>
                    <span className="detail-val">{fmtTimestamp(Number(allowData.sigDeadline))} ({fmtDuration(Number(allowData.sigDeadline) - Math.floor(Date.now() / 1000))} remaining)</span>
                    <span className="detail-key">Reusable?</span>
                    <span className="detail-val">Yes — spender can draw up to {amount} HONEY across multiple calls</span>
                  </div>
                </div>
          ...
          <>
            ...
            {method === 'allowance' && allowData && (
              <div className="detail-card time-bound-result">
                ...
                <div className="detail-grid">
                  ...
                  <span className="detail-val">Spender may still draw from the allowance if cap allows</span>
                  <span className="detail-key">Allowance expires</span>
                  <span className="detail-val">{fmtTimestamp(allowData.expiration)} ({fmtDuration(Math.max(0, allowData.expiration - Math.floor(Date.now() / 1000)))} remaining)</span>
                  <span className="detail-key">Still reusable?</span>
                  <span className="detail-val">Yes — until cap exhausted or {fmtTimestamp(allowData.expiration)}</span>
                </div>
              </div>
            )}
            ...
            <div className="detail-card transfer-summary">
              ...
              <div className="detail-grid">
                ...
                <span className="detail-val">{amount} HONEY</span>
                <span className="detail-key">Method</span>
                <span className="detail-val">{method === 'signature' ? 'One-time Signature Transfer' : 'Reusable Allowance Transfer'}</span>
