...
export function WalletDisplay({ label, description, address, honeyAddress, role }: WalletDisplayProps) {
  ...
  const { data: honeyBalance } = useReadContract({
    ...
    args: [address],
    query: {
      enabled: !!address && honeyAddress !== '0x0000000000000000000000000000000000000000',
      refetchInterval: 4000,
    },
  })
  ...
  return (
    <div className={`wallet-display ${role}`}>
      ...
      <div className="wallet-info">
        ...
        <p>
          <strong>BERA:</strong>{' '}
          {ethBalance ? parseFloat(formatEther(ethBalance.value)).toFixed(4) : '...'} BERA
        </p>
        <p>
          <strong>HONEY:</strong>{' '}
