...
EOF
    ...
    log_info "✓ Prometheus configuration generated: $prometheus_yml"
}
...
disable_subproject_vscode_configs() {
    ...
    while IFS= read -r -d '' vscode_dir; do
        if [[ -d "$vscode_dir" ]]; then
            ...
            log_info "✓ Disabled: $(basename "$(dirname "$vscode_dir")")/.vscode"
        fi
...
EOF
    ...
    log_info "✓ VS Code workspace generated: $workspace_file"
}
...
EOF
    ...
    log_info "✓ VS Code launch configurations generated: $launch_json"
}
...
EOF
    ...
    log_info "✓ VS Code tasks generated: $tasks_json"
}
...
generate_grafana_dashboard() {
    ...
    log_info "✓ Grafana dashboard generated: $dashboard_json"
}
...
generate_debug_configs() {
    ...
    echo ""
    echo "🔍 Process Detection Summary:"
        ...
        if [[ -n "$info" ]]; then
            ...
            if systemctl --user is-active "$name-cl.service" >/dev/null 2>&1; then
                ...
                if [[ -n "$cl_pid" && "$cl_pid" -gt 0 ]]; then
                    echo "     ✅ CL Process (beacon-kit): PID $cl_pid"
                else
                    echo "     ❌ CL Process: Service active but no PID found"
                fi
            else
                echo "     ❌ CL Process: Not running"
            fi
            ...
            if systemctl --user is-active "$name-el.service" >/dev/null 2>&1; then
                ...
                if [[ -n "$el_pid" && "$el_pid" -gt 0 ]]; then
                    echo "     ✅ EL Process ($el_client): PID $el_pid"
                else
                    echo "     ❌ EL Process ($el_client): Service active but no PID found"
                fi
            else
                echo "     ❌ EL Process ($el_client): Not running via systemd"
    ...
    log_info "✓ Debug configurations generated"
    echo ""
    echo "Generated files:"
    echo "  🎯 VS Code Workspace: $BERABOX_ROOT/bb-berabox.code-workspace"
    echo "  📊 Grafana Dashboard: $GRAFANA_DIR/berabox-dashboard.json"
    echo "  📈 Prometheus Config: $PROMETHEUS_DIR/prometheus.yml"
    echo ""
    echo "Next steps:"
