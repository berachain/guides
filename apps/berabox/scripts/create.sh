...
# CL: keep/cl-keys/<name>.json (priv_validator_key) and <name>.node_key.json
# EL: keep/el-keys/<name>.nodekey (discovery-secret)
# Leave blank to use ephemeral keys generated by init.
cl_key_name = ""
el_key_name = ""

[peers]
# Persistent peers for CL (CometBFT format) and EL (enode URLs).
# These are injected into CL config.toml and EL command-line arguments.
cl_persistent_peers = []
el_persistent_peers = []

[upnp]
# UPnP automatic port forwarding for P2P ports.
# Only opens P2P ports (cl_p2p_port, el_p2p_port), never RPC or admin ports.
# Requires router/gateway with UPnP support.
enabled = false
# Lease time in seconds (86400 = 24h, 0 = permanent mapping).
lease_time = 86400
EOF
