...
if command -v $RETH_BIN >/dev/null 2>&1; then
...
if command -v $NETHERMIND_BIN >/dev/null 2>&1; then
...
if command -v $ERIGON_BIN >/dev/null 2>&1; then
    export ERIGON_DATA=$(pwd)/var/erigon
    export ERIGON_GENESIS_PATH=$ERIGON_DATA/genesis.json
fi  

if ! command -v $RETH_BIN >/dev/null 2>&1 && ! command -v $GETH_BIN >/dev/null 2>&1 && ! command -v $NETHERMIND_BIN >/dev/null 2>&1 && ! command -v $ERIGON_BIN >/dev/null 2>&1; then
