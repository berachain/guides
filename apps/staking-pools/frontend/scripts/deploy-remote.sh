#!/usr/bin/env bash
# Deploy staking-pool frontend for Bepolia, Mainnet, and Rhino mainnet to cam@localhost,
...
ssh "$REMOTE" 'for p in 6900 6901 6902; do pid=$(lsof -ti:$p 2>/dev/null); [ -n "$pid" ] && kill -9 $pid 2>/dev/null || true; done
  ...
  sleep 2
  echo "Bepolia:       http://localhost:6900"
  echo "Mainnet:       http://localhost:6901"
  echo "Rhino mainnet: http://localhost:6902"
'
